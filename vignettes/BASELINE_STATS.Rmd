...
```{r, fig.height=3}
# Plot points
ggplot(NMR, aes(x = group, y = pre_weight)) +
...
wt.weight <- NMR %>% 
  ...
  mutate(1 / var(pre_weight, na.rm = TRUE)) %>% 
  pull(-1)

fit.weight <- glm(pre_weight ~ age * sex * group,
                  family = gaussian("log"),
                  weights = wt.weight,
                  data = NMR)
plot_lm(fit.weight)
...
```{r}
fit.weight.1 <- update(fit.weight, formula = . ~ age * group + sex)
anova(fit.weight.1, fit.weight, test = "F")
```

There is no significant difference between the models, so we will use the simpler one.

```{r}
fit.weight <- fit.weight.1
plot_lm(fit.weight)
```
...
Absolute VO2max is calculated by multiplying relative VO2max ($mL \cdot kg^{-1} \cdot min^{-1}$) by body weight (kg).
...
filter(VO2MAX, age == "6M") %>% 
  ggplot(aes(x = pre_weight, y = pre_vo2max_ml_min)) +
  geom_point(na.rm = TRUE, alpha = 0.5)
```

That outlier has one of the highest weights, though it is not unusual. We will determine the mean–variance relationship.
...
speed_res <- VO2MAX %>% 
  ...
  dplyr::select(age, sex, contrast, estimate, lower.CL, upper.CL,
                starts_with("n_"), W.ratio, p.value)
```
...
BASELINE_STATS <- c(other_stats, vo2max_stats,
