...
[@aitchison_compositional_1982; @boogaart_compositional_2013]
...
FIBER_COUNT_EMM <- map(fit.count[1:4], function(mod_i) {
  by <- c("age", "sex")
  mult.name <- NULL
...
FIBER_COUNT_MVT <- map(FIBER_COUNT_EMM, function(emm_i) {
  ...
  mutate(p.adj = p.adjust(p.value, method = "holm"),
         ...
         across(.cols = c(contains("ratio"), T.square, estimate, SE), 
                .f = ~ round(.x, digits = 2)),
         across(.cols = c(p.value, p.adj), .f = ~ signif(.x, digits = 2)))
```
...
FIBER_COUNT_STATS <- map(FIBER_COUNT_EMM, function(emm_i) {
  ...
  relocate(muscle, balance, .before = contrast) %>% 
  group_by(age, sex, muscle) %>% 
  mutate(p.adj = p.adjust(p.value, method = "holm")) %>% 
  ungroup() %>% 
  mutate(across(.cols = c(estimate, SE, lower.CL, upper.CL, t.ratio), 
                .f = ~ round(.x, digits = 2)),
         # Mark significant comparisons (adjusted p-value < 0.05)
         signif = ifelse(p.adj < 0.05, "*", ""),
         across(.cols = c(p.value, p.adj), 
                .f = ~ signif(.x, digits = 2))) %>%
